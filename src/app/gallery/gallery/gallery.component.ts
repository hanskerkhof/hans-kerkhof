...
export class GalleryComponent implements OnInit, OnDestroy, OnChanges {
  ...
  // tslint:disable-next-line: no-input-rename
  @Input('metadataUri') providedMetadataUri = undefined;
  // tslint:disable-next-line: no-input-rename
  ...
  @HostListener('window:scroll', ['$event']) triggerCycle(): void {
    this.scaleGallery();
  }

  @HostListener('window:resize', ['$event']) windowResize(): void {
    this.render();
  }
  ...
  navigate(direction: number): void {
    if (
      (direction === 1 &&
        this.rowIndex < this.gallery.length - this.rowsPerPage) ||
    ...
    if (!this.providedMetadataUri) {
      this.imageDataCompletePath =
        this.providedGalleryName !== ''
          ? `${this.imageDataStaticPath + this.providedGalleryName}/${
              this.dataFileName
            }`
  ...
  private render(): void {
    ...
    for (i; i < this.images.length; i++) {
      while (
        this.images[i + 1] &&
        this.shouldAddCandidate(tempRow, this.images[i + 1])
      ) {
        i++;
      }
      if (this.images[i + 1]) {
  ...
  private calcOriginalRowWidth(imgRow: Array<any>): number {
    ...
    imgRow.forEach((img) => {
      const individualRatio =
        this.calcIdealHeight() / img[this.minimalQualityCategory].height;
  ...
  private scaleGallery(): void {
    ...
    this.gallery
      .slice(this.rowIndex, this.rowIndex + this.rowsPerPage)
      .forEach((imgRow) => {
        const originalRowWidth = this.calcOriginalRowWidth(imgRow);
        ...
        if (imgRow !== this.gallery[this.gallery.length - 1]) {
          const ratio =
            (this.getGalleryWidth() -
              (imgRow.length - 1) * this.calcImageMargin()) /
            originalRowWidth;
          ...
          imgRow.forEach((img: any) => {
            ...
            maximumGalleryImageHeight = Math.max(
    ...
    this.minimalQualityCategory =
      maximumGalleryImageHeight > 375 ? 'preview_xs' : 'preview_xxs';
    this.refreshNavigationErrorState();
